...
//	private static final String userHome = System.getProperty("user.home").replaceAll("\\\\", "/");
	...
	public Configuration sqliteConfigurationConnection(String databaseName) {
			...
			configuration = new Configuration()
				...
				.addAnnotatedClass(RealtorDotComProperty.class)
...
//			    .setProperty("hibernate.connection.password", password)
	...
	public synchronized void insertIntoDatabase(RealtorDotComProperty realtorDotComProperty) {
		
		transaction = session().beginTransaction();
		
		session.persist(realtorDotComProperty);
